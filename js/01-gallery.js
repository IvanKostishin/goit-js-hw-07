...
function onParentClick(evt) {
  ...
  if (evt.target.nodeName !== "IMG") {
    ...
    const instance = basicLightbox.create(`
        ...
        onShow: () => document.addEventListener("keydown", function(event){if (event.code === "Escape") {
         instance.close();
    }}),
        onClose: () => document.removeEventListener("keydown", function(event){if (event.code === "Escape") {
         instance.close();
    }}),
    }
)
